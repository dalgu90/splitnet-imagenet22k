    ...
    with tf.variable_scope(name):
        ...
        with tf.device('/CPU:0'):
            if init_k is not None:
                ...
                initializer_k =tf.random_normal_initializer(stddev=np.sqrt(2.0/filter_size/filter_size/out_channel))
            kernel = tf.get_variable('kernel', [filter_size, filter_size, in_shape[3], out_channel],
    ...
    with tf.variable_scope(name):
        ...
        with tf.device('/CPU:0'):
            if init_w is not None:
                ...
                initializer_w = tf.random_normal_initializer(stddev=np.sqrt(1.0/out_dim))
