...
RESNET_MEAN_FPATH = 'ResNet_mean_bgr.pkl'
...
def preprocess_image(input_image):
  ...
  image_R, image_G, image_B = tf.split(image, 3, 2)

  # 1) Subtract channel mean
  # blue_mean = 103.062624
  # green_mean = 115.902883
  # red_mean = 123.151631
  # image = tf.concat([image_B - blue_mean, image_G - green_mean, image_R - red_mean], 2, name="centered_bgr")

  # 2) Subtract per-pixel mean(the model have to 224 x 224 size input)
  image = tf.concat([image_B, image_G, image_R], 2) - resnet_mean
...
def ten_crop_inputs(dataset_root, txt_fpath, batch_size, num_threads=1):
  for f in [dataset_root, txt_fpath]:
    if not gfile.Exists(f):
      raise ValueError('Failed to find file: ' + f)
  ...
  # Read examples from files.
  read_input = read_input_file(txt_fpath, dataset_root, shuffle=False)
  ...
  # Preprocess the image: tf.float32 cast & mean subtract
  image = preprocess_image(image)

  # offsets
  center_h_offset = tf.cast((image_height-crop_height)/2, tf.int32)
  center_w_offset = tf.cast((image_width-crop_width)/2, tf.int32)
  offset_list = [(0, 0), (0, image_width-crop_width), (image_height-crop_height, 0),
                 (image_height-crop_height, image_width-crop_width), (center_h_offset, center_w_offset)]

  # 10-crop
  ten_crop_images = []
  for h_offset, w_offset in offset_list:
    crop_image = tf.slice(image, [h_offset, w_offset, 0], [crop_height, crop_width, 3])
    ten_crop_images.append(crop_image)
  image = tf.image.flip_left_right(image)
  for h_offset, w_offset in offset_list:
    crop_image = tf.slice(image, [h_offset, w_offset, 0], [crop_height, crop_width, 3])
    ten_crop_images.append(crop_image)

  ten_crop_images = tf.concat([tf.expand_dims(img, 0) for img in ten_crop_images], axis=0)
  ten_crop_labels = tf.concat([tf.expand_dims(read_input.labels, 0) for _ in range(10)], axis=0)

  # Generate a batch of images and labels by building up a queue of examples.
  image_batch, label_batch = tf.train.batch(
        [ten_crop_images, ten_crop_labels],
        enqueue_many=True,
